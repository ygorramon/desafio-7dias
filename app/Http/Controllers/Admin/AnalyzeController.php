...
return view ('admin.pages.analyzes.index', ['clients'=>$clients]);
   ...
   public function processar ($id){
      ...
      $qtd_janelas_rituals_inadequadas_fim=count($client->rituals->where('window','>',$janela->janelaIdealFim));
      $qtd_janelas_rituals_inadequadas_inicio=count($client->rituals->where('window','<',$janela->janelaIdealInicio));
